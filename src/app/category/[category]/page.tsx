  ...
  return (
    <div className="min-h-screen relative ">
      <div className=' bg-PureBlack-100 relative text-PureWhite-100'>
         <h1 className='p-6 font-bold tracking-widest text-xl pt-23 lg:pt-40 uppercase text-center md:py-20 md:font-bold md:text-3xl'>{category}</h1>
      </div>
      <div className='py-10 px-4 lg:p-35 lg:px-35 grid grid-cols-1 gap-10 md:pt-25 md:px-10'>
        {products.map((product: Product,index:number) => (
          <div key={product.id} className="flex space-y-4 text-center lg:justify-between flex-col md:gap-2 lg:flex-row items-center mb-6">
            ...
            {index===0&&<h2 className="text-xs lg:text-start tracking-[5px] md:tracking-[10px] lg:tracking-[7px] lg:text-lg font-light text-BurntSienna-100 mt-3">NEW PRODUCT</h2>}
            <h2 className="text-lg w-3/5 lg:text-left lg:w-3/5 uppercase tracking-wide md:w-1/2 md:font-bold md:text-3xl lg:text-4xl font-semibold">{product.name}</h2>
