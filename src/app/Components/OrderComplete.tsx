       ...
       <div onClick={handleBackgroundClick} className=" fixed flex w-full items-center justify-center z-50 bg-PureBlack-100/40 top-0 min-h-screen">
        ...
        <div className="rounded-md md:flex md:w-full overflow-hidden">
            <div className=" bg-OffWhite-100 flex flex-col p-4 justify-center md:w-3/5">
                ...
                {cartItems.length > 1 && (
                    <div className="border-t-1 font-bold lg:font-semibold lg:text-xs text-center md:text-sm border-Gray-200 my-2 pt-2 md:pt-4 lg:pt-2 text-xs text-Gray-200">
        ...
        <Button text="BACK TO HOME" link="/" onClick={handleButtonClick} className="w-full md:py-4 lg:py-2 font-bold"/>
        </div>
        </div>
  );
};
